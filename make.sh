...
case "$1" in
    pr | submit) 
        ...
        universe_dir_pref=../typst-packages-universe/packages/preview/introterm
        universe_dir=../typst-packages-universe/packages/preview/introterm/"$VER"
    ...
    install_local | i )
        ...
        VER="$(tomlq -r .package.version src/typst.toml)"
        rsync -auv --delete --mkpath src/ "$HOME"/.local/share/typst/packages/local/introterm/"$VER"
        ;;
    fast | f )
